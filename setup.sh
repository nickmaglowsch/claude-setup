...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -h|--help)
      ...
      echo "  --cross-review            Set up cross-model (Codex/GPT) review (installs the"
      echo "                            Codex CLI + SessionStart self-heal hook + review helpers)"
      echo ""
...
# --- Cross-model review: register the Codex self-heal SessionStart hook ---
# Merges into ~/.claude/settings.json without clobbering existing hooks/settings.
_install_codex_session_hook() {
  ...
  if [ -f "$settings_file" ] && grep -q 'ensure-codex' "$settings_file" 2>/dev/null; then
    return 0
  fi
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash ~/.claude/scripts/ensure-codex.sh"
          }
        ]
      }
    ]
...
HOOK_EOF
    echo "  Registered Codex SessionStart self-heal hook."
    return 0
  fi

  # Merge into existing settings.json — python3 first, jq fallback.
  local merged
  merged=$(python3 -c "
import json, sys
with open(sys.argv[1]) as f:
...
for entry in ss:
    for h in entry.get('hooks', []):
        if 'ensure-codex' in h.get('command', ''):
            print(json.dumps(data, indent=2))
            sys.exit(0)
ss.append({
    'matcher': '',
    'hooks': [{'type': 'command', 'command': 'bash ~/.claude/scripts/ensure-codex.sh'}]
})
print(json.dumps(data, indent=2))
" "$settings_file" 2>/dev/null) && {
    echo "$merged" > "$settings_file"
  ...
  if command -v jq &>/dev/null; then
    merged=$(jq '
      .hooks //= {} |
      .hooks.SessionStart //= [] |
      if (.hooks.SessionStart | map(.hooks[]?.command // "" | test("ensure-codex")) | any)
      then .
      else .hooks.SessionStart += [{"matcher": "", "hooks": [{"type": "command", "command": "bash ~/.claude/scripts/ensure-codex.sh"}]}]
      end
    ' "$settings_file" 2>/dev/null) && {
      echo "$merged" > "$settings_file"
      echo "  Registered Codex SessionStart self-heal hook."
  ...
  echo "  Warning: could not register Codex SessionStart hook (python3 and jq unavailable)."
  echo "  Add manually to ~/.claude/settings.json under hooks.SessionStart:"
  echo "    {\"matcher\": \"\", \"hooks\": [{\"type\": \"command\", \"command\": \"bash ~/.claude/scripts/ensure-codex.sh\"}]}"
  return 1
}

# --- Cross-Model Review (Codex / GPT) bootstrap ---
# Optional, opt-in. Installs the Codex CLI, the review helper scripts, and the
# SessionStart self-heal hook. Idempotent and non-fatal at every step.
_setup_cross_review_bootstrap() {
  # Already configured (hook registered): refresh helper scripts silently and
  # return — keeps global updates from re-prompting and stays idempotent.
  if [ -f "$HOME/.claude/settings.json" ] && grep -q 'ensure-codex' "$HOME/.claude/settings.json" 2>/dev/null; then
    echo "=== Cross-Model Review (Codex / GPT) ==="
    echo "  Already configured — refreshing review helper scripts."
    mkdir -p "$HOME/.claude/scripts"
    for s in ensure-codex.sh codex-review.sh; do
      if [ -f "$SCRIPT_DIR/scripts/$s" ]; then
        cp "$SCRIPT_DIR/scripts/$s" "$HOME/.claude/scripts/$s"
        chmod +x "$HOME/.claude/scripts/$s" 2>/dev/null || true
      fi
    done
    echo ""
    return 0
  ...
  echo "=== Cross-Model Review (Codex / GPT) ==="
  echo ""
  echo "  Adds GPT (via the Codex CLI) as a decorrelated second reviewer and rescue"
  echo "  for the /build, /refactor, and /debug-workflow pipelines. Claude stays the"
  echo "  primary reviewer; Codex augments it. All review calls are read-only and fail"
  echo "  soft — they can never block a pipeline. Codex runs on your ChatGPT"
  echo "  subscription and consumes its quota separately."
  echo ""
  read -rp "  Set up cross-model review now? [y/N] " cr_choice
  echo ""

  if [[ ! "$cr_choice" =~ ^[Yy]$ ]]; then
    echo "  Skipped. Re-run anytime with: ./setup.sh --cross-review"
    echo ""
    return 0
  fi

  # --- Codex CLI (idempotent) ---
  if command -v codex &>/dev/null; then
    ...
    if command -v npm &>/dev/null; then
      ...
      npm install -g @openai/codex 2>&1 | sed 's/^/    /' || \
        echo "    Warning: Codex install failed — install manually: npm install -g @openai/codex"
    else
      echo "  npm not found — install Node/npm, then run: npm install -g @openai/codex"
    fi
  fi
  echo ""

  # --- Optional interactive Codex plugin (best-effort, non-fatal) ---
  # The pipelines call `codex exec` directly and do NOT depend on this plugin;
  # it is installed only as manual-use sugar. Failures are ignored.
  if command -v claude &>/dev/null; then
    echo "  Registering optional interactive Codex plugin (best-effort)..."
  ...
  # --- Review helper scripts → ~/.claude/scripts/ ---
  mkdir -p "$HOME/.claude/scripts"
  for s in ensure-codex.sh codex-review.sh; do
    if [ -f "$SCRIPT_DIR/scripts/$s" ]; then
      cp "$SCRIPT_DIR/scripts/$s" "$HOME/.claude/scripts/$s"
      chmod +x "$HOME/.claude/scripts/$s" 2>/dev/null || true
      echo "  Installed ~/.claude/scripts/$s"
    fi
  done
  ...
  if [ ! -f "$HOME/.codex/auth.json" ]; then
    echo "  ONE-TIME SETUP: run 'codex login' to authenticate Codex with your ChatGPT subscription."
    echo "  (Cross-model review fails soft until this is done — pipelines still run.)"
  fi
  echo ""
}
...
# Cross-review only: run just the cross-model review bootstrap and exit
if [ "$CROSS_REVIEW_ONLY" = true ]; then
  _setup_cross_review_bootstrap
  exit 0
fi
...
# --- Step 3d: Cross-Model Review (Codex / GPT) (optional) ---
_setup_cross_review_bootstrap
