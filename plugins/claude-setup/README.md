...
Use the plugin when you want versioned, shareable, namespaced workflows without running the shell installer. The plugin packages Claude Code skills and agents only.
...
Installing this plugin does not modify `~/.claude/settings.json`, install RTK, add hooks, create cron jobs, install the devcontainer, or add `run-claude.sh`.
